	...
	<form action="StudentUpdateExecute.action" method="get">
		<div style="padding: 20px;">
			...
			<div style="margin-bottom: 15px;">
				...
				　<input readonly type="text" name="ent_year" value="${ent_year}" style="border: 0;">
			</div>

			<div style="margin-bottom: 15px;">
				<label class="form-label"
				 style="margin-bottom: 0;">学生番号</label><br>
				　<input readonly type="text" name="no" value="${no}" style="border: 0;">
			</div>

			<div>
				<label class="form-label">氏名</label>
				<input class="form-select"  type="text" name="name" value="${name}" required
			...
			<div>
				...
				<select class="form-select" id="student-f2-select" name="class_num">
					<c:forEach var="c" items="${class_num_set}">
						<option value="${c}" <c:if test="${c==num}">selected</c:if>>${c}</option>
					</c:forEach>
				</select>
			</div>
