...
public class TestListSubjectDao extends Dao {
	...
	public boolean delete(Student student) throws Exception {
		...
		try {
			...
			statement = connection.prepareStatement("update student set is_attend=false, deleted=true where no=?");
			// プリペアードステートメントに学生番号をバインド
			statement.setString(1, student.getNo());
			// プリペアードステートメントを実行
			count = statement.executeUpdate();
	...
	/**
	 * enterメソッド 受け取った入学年度の学生の在学フラグをtrueに一括変更
	 *
	 * @param student：Student
	 *            学生
	 * @return 成功:true, 失敗:false
	 * @throws Exception
	 */
	public boolean enter(int entYear, String schoolCd) throws Exception {
		// コネクションを確立
		Connection connection = getConnection();
		// プリペアードステートメント
		PreparedStatement statement = null;
		...
		try {
			...
			// プリペアードステートメントにUPDATE文をセット
			statement = connection
					.prepareStatement("update student set is_attend=true where ent_year=? and school_cd=?");
			// プリペアードステートメントに値をバインド
